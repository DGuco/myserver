...
#ifdef __LINUX__
	/*
		在调用进程处理器函数时，内核通常会在进程栈中为其创建一栈。不过，如果进程对栈的扩展突破了对栈大小的限制时，这种做法就不太可行了。
		比如，栈的增长过大，以至于会触及到一片映射内存或者向上增长的堆，又或者栈的大小已经直逼RLIMIT_STACK资源限制，这些都会造成这种情
		况的发生。当进程对栈的扩展试图突破其上限时，内核将为该进程产生 SIGSEGV信号号。不过，因为栈空间已然耗尽，内核也就无法为进程已经
		安装的SIGSEGV 处理器函数创建栈帧。结果是，处理器函数得不到调用，而进程也就终止了。如果希望在这种情况下确保对 SIGSEGV 信号处理
		器函数的调用，就需要做如下工作分配一块被称为“备选信号栈”的内存区域，作为信号处理函数的栈帧调用sigaltstack() ，告知内核该备选信
		号栈的存在在创建信号处理函数时指定SA_ONSTACK 标志，也就是通知内核在备选栈上为处理器函数创建栈帧。利用系统调用 sigaltstack()，
		既可以创建一个备选信号栈，也可以将已创建备选信号栈的相关信息返回
	*/
	stack_t ss;

...
public:
	...
	void initialize()
	{
		...
		super::initailize_pool();
		LOG_NOTICE("default", "init addr 0x%08x", this);
	}

	void resume()
	{
		super::resume_pool();
		LOG_NOTICE("default", "resume addr 0x%08x", this);
	}
	...
	*/
	...
	{
		int count1 = 0;
		for (; last_tick_obj_ != super::end() && count1 <= max_num;
			++last_tick_obj_, ++count1)
		{
	...
	*         如果失败，返回0
	*/
	pointer find_by_index(int index)
	{
		if (index >= 0 && index < (int)super::max_size() )
		{
			pointer p = super::operator [](index);
			if (p) 
			{
				return p;
			}

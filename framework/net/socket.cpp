...
//----------------------------------------------------------------
bool CSocket::Open() {
  GH_ASSERT(INVALID_SOCKET == m_Socket);
  m_Socket = CreateSocket(m_nType, m_nProtocolFamily, m_nProtocol);
  if (INVALID_SOCKET == m_Socket) {
	GH_DEBUG_ASSERT(false, "CreateSocket failed with error code %d \n", PpeGetLastError());
  }
  return true;
}
...
SOCKET CSocket::CreateSocket(int32 Type, int32 ProtocolFamily, int32 Protocol) {
  ...
  if (Socket == INVALID_SOCKET) {
	GH_DEBUG_ASSERT(false, "socket failed with error code %d .", PpeGetLastError());
  }
  int iVal = 1;
  if (SOCKET_ERROR ==
	  setsockopt(Socket, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char *>(&iVal), sizeof(iVal))) {
	GH_DEBUG_ASSERT(false, "setsockopt failed with error code %d .", PpeGetLastError());
  }
  return Socket;
}
...
int CSocket::GetSocketError() const {
  ...
  default:GH_DEBUG_ASSERT(false, "unknown failed!!");
  }
  return eReason;
...
void CSocket::MakeSocketNonblocking(SOCKET Socket) {
  if (-1 == evutil_make_socket_nonblocking(static_cast<int>(Socket))) {
	CloseSocket(Socket);
	GH_DEBUG_ASSERT(false, "ioctlsocket failed with error code %d.", PpeGetLastError());
  }
}
...
int CDBHandle::InitLogFile(const char *vLogName,
						   const char *vLogDir,
						   level_enum vPriority  /*= LEVEL_NOTSET*/ ,
...
int CDBHandle::RunFunc()
{
	...
	while (True) {
		...
		if (iTempRet < 0) {
			LOG_ERROR("default", "Get one code from input queue returns {}.\n", iTempRet);
			continue;
		}

		if (nTempCodeLength <= 0) {
			LOG_ERROR("default", "Got one empty code from input queue, ignore it.\n");
			continue;
		}

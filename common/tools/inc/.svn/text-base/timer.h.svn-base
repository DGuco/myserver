...
public:
	// 定时器类型
	ETimerType  GetTimerType() {return m_eTimerType;}
	// 超时时间(单位 : 毫秒)
	time_t GetTimeout() {return m_tTimeout;}
	// 扩展参数个数(无法设置,只能调用SetOtherInfos接口重新设置参数)
	int  GetOtherInfoNum() {return m_iOtherInfoNum;}
	// 获取扩展参数
	...
	// 设置定时器待销毁状态
	void NeedDestroy() {mbDestroy = true;}
...
public:
	// 定时器类型
	int  GetTimerMark() {return m_iTimerMark;}
	// 定时器时间运行模式
	ERunTimeMode  GetRunTimeMode() {return m_eRunTimeMode;}
	// timer拥有者的实体ID
	int  GetOwnerEntityID() {return m_iOwnerEntityID;}
	// 单次间隔时间(单位 : 毫秒,用于计算下次Timeout的值)
	time_t GetMillTime() {return m_tMillTime;}
	// timer执行次数(大于0则按此执行,小于0则为永久执行)
	int  GetCalledNum() {return m_iCalledNum;}
	// 获取扩展参数
	int GetOtherInfo(int iIdx);
	// 设置timer信息（返回Timer的ID）Owner可以为空,主要为了确认玩家关联时使用
	void SetTimer(int iTimerMark, ERunTimeMode enRunTimeMode, time_t tMillTime, CallFunc func, int iCalledNum, int iOwnerEntityID, unsigned long ulOwnerID);
	...
	// timer拥有者的ID
	unsigned long GetOwnerID() {return m_ulOwnerID;}
...
//	void SetSession(time_t tMillTime, CallFunc func, int iOwnerEntityID, unsigned long ulOwnerID, bool bContinues = false);
	void SetSession(time_t tMillTime, CallFunc func, int iOwnerEntityID, unsigned long ulOwnerID);
	// 设置session扩展参数（返回值 0：成功 < 0：失败）
	int SetOtherInfos(int iNum, int* piParams);
	// 超时处理
	void Timeout(time_t tNow);
//	// 是否可持续
//	bool IsContinues() {return m_bContinues;}
...
protected:
	...
	int	m_iOwnerEntityID;		// 创建session的玩家的实体ID
	unsigned long m_ulOwnerID;		// 创建session的玩家的id(玩家创建时才需要,其它不需要该参数)
...
protected:
	...
	typedef yq::hash_map<time_t, CDoubleLinkerInfo, FINDER_QUEUE_LENGTH> TIMER_FINDER;
	typedef yq::vector<int, FINDER_QUEUE_LENGTH>	DELETE_LIST;
	...
	// 删除实体
	int DeleteObject(int iObjID);
	// 真实处理定时器实体销毁的接口
	void RealDestroyTimer(int iObjID);
...
public:
	...
	int DestroyObject(int iObjID);
	// 提供给外部使用的获取实体的接口
	CObj* GetObject(int iObjID);

	// ----------------------------------------------------------------------------- //
	// CTimer相关接口
	// ----------------------------------------------------------------------------- //
	// 创建timer
	CTimer* CreateTimer(int iTimerMark, CTimer::ERunTimeMode enRunTimeMode, time_t tMillTime, CTimer::CallFunc func, int iCalledNum = CTimer::TIMER_FOREVER, time_t tRemainTime = 0, int iOwnerEntityID = 0, unsigned long ulOwnerID = 0);
	// timer强制超时（bClear表示本次超时以后是否直接清除timer）
	void ForceTimeout(int iTimerID, bool bClear = false);

	// ----------------------------------------------------------------------------- //
	// CSission相关接口
	// ----------------------------------------------------------------------------- //
	// 创建session
	CSession* CreateSession(time_t tMillTime, CSession::CallFunc func, int iOwnerEntityID = 0, unsigned long ulOwnerID = 0);

/******************************************************************************
	...
	修改内容	: 生成
*******************************************************************************/
#ifndef __YQ_SHAREVECTOR_H__
#define __YQ_SHAREVECTOR_H__
...
namespace yq
{
	...
	{
		typedef yq::array_list_node_base<T,L> node_type;
	...
	public:
		...
		typedef yq::fixed_size_memory_pool<T, count, 2> shared_array_list;
		...
		iterator push_back(const T & t)
		{
			...
			if( p != NULL )
			{
				...
				if( begin__ == -1 && end__ == -1 )
				{
					...
					end__ = begin__;
					assert( begin__ >=0 && end__ >= 0 && begin__ < (int)count && end__ < (int)count );

					//current_node->other_link_[0] = 0;
					//current_node->other_link_[1] = 0;
					current_node->set_other_link(0, 0);
					current_node->set_other_link(0, 1);
				}
				else
				{
					...
					//current_node->other_link_[1] = 0;
					end_node->set_other_link(current_node - end_node, 1);
					current_node->set_other_link(end_node - current_node, 0);
					current_node->set_other_link(0, 1);

					end__ = share_list_ptr__->node_offset(p);
					assert( end__ >= 0 && end__ < (int)count );
				}
		...
		iterator erase(iterator pos)
		{
			...
			if (pos != endPos)
			{
				...
				else if ( is_last )
				{
					...
					end_node->set_other_link(0, 1);
				}
				else
				{
					//TODO: 这里没有做安全检查，iterator必须保证来自本vector,　否则会把其他vector的对象删除
					node_type* curr_node = reinterpret_cast<node_type*>(&*pos);
					//offset_type curr_off = share_list_ptr__->node_offset(&*pos);
					assert( share_list_ptr__->node_offset(&*pos) >= 0 && share_list_ptr__->node_offset(&*pos) < (int)count );
					...
					prev_node->set_other_link(offset, 1);
					next_node->set_other_link(-offset, 0);
	...
	template < typename T, std::size_t count >
	yq::fixed_size_memory_pool<T, count, 2>* shared_vector<T,count>::share_list_ptr__ = NULL;


};

#endif // __YQ_SHAREVECTOR_H__

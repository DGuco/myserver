...
int CClientHandle::DecodeNetMsg(BYTE* pCodeBuff, PACK_LEN& nLen, MesHead* pCSHead, CMessage* pMsg)
{
    ...
    if (tmpSocketInfo.state() < 0)
    {
        ...
        else
        {
            ...
            if (!connectInfo && (connectInfo->m_iSocket != iTmpSocket || connectInfo->m_tCreateTime != tTmpCreateTime))
            {
                // 当前玩家与该连接信息不匹配,说明该玩家的连接已经失效
                LOG_WARN("default", "[%s : %d : %s] socket(%d : %d) not match, now(%d : %d).",
                         __MY_FILE__, __LINE__, __FUNCTION__, iTmpSocket, tTmpCreateTime, connectInfo->m_iSocket, connectInfo->m_tCreateTime);
                // 重置玩家连接信息
        ...
        if( pTmpTeam)
        {
            ...
            else
            {
                ...
                LOG_INFO("default", "[%s : %d : %s]  Socket(%d) is used, old team(TeamID=%lu, EntityID=%d), reset.",
                         __MY_FILE__, __LINE__, __FUNCTION__, iTmpSocket, pTmpTeam->GetTeamID(), pTmpTeam->GetEntityID());

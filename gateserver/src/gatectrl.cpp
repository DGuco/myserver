...
void CGateCtrl::BeginListen()
{
    ServerInfo *gateInfo = CServerConfig::GetSingletonPtr()->GetServerInfo(enServerType::FE_GATESERVER);
    //初始化epoll
    m_pNetWork->BeginListen(gateInfo->m_sHost.c_str(),
    ...
    for (i = 0, pevents = m_pEpollevents; i < iTmpfdNum; i++, pevents++) {
        ...
        if (m_pSocketInfo->m_iSocketType == LISTEN_SOCKET) {
            ...
            if (ioctl(iTmpNewSocket, FIONBIO, &iTmpFlags) &&
                ...
                LOG_ERROR("default",
                          "operate on socket %d error connect port %d!",
                          iTmpNewSocket,
                          m_pSocketInfo->m_iConnectedPort);
                CloseSocket(iTmpNewSocket);
                continue;
            }
            ...
            if (iTmpRet != 0) {
                LOG_ERROR("default",
                          "add to epoll failed [socket %d connect port %d]!",
                          iTmpNewSocket,
                          m_pSocketInfo->m_iConnectedPort);
                CloseSocket(iTmpNewSocket);
                continue;
            ...
            LOG_NOTICE("default",
                       "%s connected port %d, socket id = %d.",
                       pTmpIp,
                       m_pSocketInfo->m_iConnectedPort,
                       iTmpNewSocket);
        }
...
static void CGateCtrl::OnAcceptCns(uint32 uId, CAcceptorEx *pAcceptorEx)
{
    std::cout << "OnAcceptCns : " << uId << std::endl;
}

static void CGateCtrl::OnCnsDisconnected(CAcceptorEx *pAcceptorEx)
{
}

static void CGateCtrl::OnCnsSomeDataSend(CAcceptorEx *pAcceptorEx)
{
}
